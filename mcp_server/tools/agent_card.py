...
DEFAULT_AGENT_ENDPOINTS: List[str] = [
    ...
    for url in os.environ.get(
        "AGENT_ENDPOINTS",
        "http://agent_echo_server:28001,http://agent_langchain_server:28003",
    ).split(",")
    if url.strip()
]
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if "supportedInterfaces" not in card_data or not card_data["supportedInterfaces"]:
                ...
                if curr_url in ["http://localhost:8000", "http://0.0.0.0:8000"]:
...
def register_tools(mcp: FastMCP) -> None:
    @mcp.tool()
    async def get_agent_card(base_url: str = "http://agent_echo_server:28001") -> dict:
        """
